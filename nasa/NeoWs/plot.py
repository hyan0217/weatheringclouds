...
class AsteroidPlot:
    ...
    def plot_asteroids(self, neos_list: list):
        ...
        names = [(name + " {:.2f}".format(dia) + " M wide")
                 for (name, miss_time, miss_dist, dia, url) in neos_list]
        # We will add the moon to our plot to help give a sense of scale
        names.append('Moon')

        # Drop the nanoseconds we don't need off the end of the timestamps
        x = [datetime.datetime.strptime(miss_time[0:-7], '%Y-%m-%d %H:%M:%S')
             for (name, miss_time, miss_dist, dia, url) in neos_list]
        x.append(datetime.datetime.strptime(self.date_str + ' 04:00',
                 '%Y-%m-%d %H:%M'))  # We'll put the moon at 4 hours
        ...
        # This is an arbitrary size, since the moon is actually much larger than any of our NEOs
        radii.append(1e6)
        ...
        max_size = max(sizes)
        # Moon is 2,158.8 miles in diameter
        scale = " {:.2f}".format(2158.8/max_size)
        ...
        output_file("asteroids.html", title="NEOs Demo", mode="cdn")
        ...
        p.circle(x="hours", y="distance", source=source, radius="radii",
                 fill_color="colors", fill_alpha=0.6, line_color=None)

        labels = LabelSet(x='hours', y='distance', text='names',
